/** Javascript/PHP Spell Checker v1.3.2 https://github.com/LPology/Javascript-PHP-Spell-Checker Copyright 2012-2013 LPology, LLC Released under the MIT license */
...
sc.encodeHTML=function(a){return a.replace(/[&<>"']/g,function(a){return"&"+{"&":"amp","<":"lt",">":"gt",'"':"quot","'":"#39"}[a]+";"})};
...
this._uniqueID).style.display="block",this._canUndo=!1,this._isOpen=!0,sc.addEvent(document,"keyup",this._closeOnEsc),this._notifyMsg("checking"),this._sendData())},_closeChecker:function(){!0===this._isOpen&&(sc._("spell-msg"+this._uniqueID).style.display="none",sc._("spell-modal"+this._uniqueID).style.cssText="z-index:4999; display:none;",sc._("spell-overlay"+this._uniqueID).style.display="none",sc._("spell-current"+this._uniqueID).value="",sc._("spell-context"+this._uniqueID).innerHTML="",sc._("spelling-suggestions"+
this._uniqueID).options.length=0,this._isOpen=this._previousMatchOffset=this._previousWordMatches=this._undoPrevious=this._canUndo=this._matchOffset=this._wordMatches=this._currentWord=this._wordKeys=this._wordObject=this._text=null,sc.removeEvent(document,"keyup",this._closeOnEsc))},_showReviewer:function(){sc._("spell-msg"+this._uniqueID).style.display="none";sc._("spell-modal"+this._uniqueID).style.zIndex=5001},_notifyMsg:function(a){var b,c=sc._("spell-msg-close-box"+this._uniqueID);"checking"==
a?(b="Checking...",c.style.display="none"):c.style.display="block";"servererror"==a&&(b="We have experienced an error and cannot complete the spell check.");"noerrors"==a&&(b="Spell check completed. No errors found.");"finished"==a&&(b="Spell check completed");sc._("spell-modal"+this._uniqueID).style.zIndex=4999;sc._("spell-msg-text"+this._uniqueID).innerHTML=b;sc._("spell-msg"+this._uniqueID).style.display="block"},_ignoreChange:function(a){var b=this._wordMatches,c=this._matchOffset;!0===a||1>=
b||c===b?(this._wordKeys.splice(0,1),this._matchOffset=1,a=!1):(this._matchOffset=c+1,a=!0);sc._("spell-undo"+this._uniqueID).setAttribute("disabled",!0);this._canUndo=!1;0<this._wordKeys.length||!0===a?this._reviewWord():this._notifyMsg("finished")},_makeChange:function(a){var b=this._text,c=this._currentWord,d=this._wordMatches,e=this._matchOffset,f=sc._("spelling-suggestions"+this._uniqueID),l=f.selectedIndex,m=RegExp(c,"g"),k=0,h;sc._("spell-hidden"+this._uniqueID).value=b;this._undoPrevious=
c;this._previousWordMatches=d;this._previousMatchOffset=e;sc._("spell-undo"+this._uniqueID).removeAttribute("disabled");this._canUndo=!0;h=-1<l?f.options[l].text:sc._("spell-current"+this._uniqueID).value;b=b.replace(m,function(b){k++;return!0===a||e===k?h:b});!0===a||1>=d||e===d?(this._wordKeys.splice(0,1),this._matchOffset=1,d=!1):d=!0;this._text=this._textInput.value=b;0<this._wordKeys.length||!0===d?this._reviewWord():this._notifyMsg("finished")},_undoChange:function(){var a=this._undoPrevious,
b=sc._("spell-hidden"+this._uniqueID),c=b.value;this._text=this._textInput.value=c;this._currentWord=sc._("spell-current"+this._uniqueID).value=a;sc.contains(this._wordKeys,a)||this._wordKeys.unshift(a);this._wordMatches=this._previousWordMatches;this._matchOffset=this._previousMatchOffset;this._canUndo=!1;sc._("spell-undo"+this._uniqueID).setAttribute("disabled",!0);b.value="";this._setSuggestionOptions();this._setContextBox()},_setSuggestionOptions:function(){var a=this._wordObject,b=this._currentWord,
c=sc._("spelling-suggestions"+this._uniqueID),a=a[b],b=a.length,d;c.options.length=0;if(!(1>b)){for(d=0;d<b;d++)c.options[d]=new Option(a[d],a[d]);c.selectedIndex=0}},_setContextBox:function(){var a=this._currentWord,b=a.length,c=this._matchOffset,d=this._text,e=d.length,f=sc._("spell-context"+this._uniqueID),l=/^\w+$/,m=0;d.replace(RegExp(a,"g"),function(k,h){if(l.test(d.substr(h+b,1)))return k;m++;if(m===c){var j,g;j=h-20;g=h+b;j=0>j?d.substr(0,h):d.substr(j,20);g=g+50>e?d.substr(g):d.substr(g,
50);j=j.replace(/^\s+/,"").replace(/[^\s]+/,"").replace(/^\s+/,"");g=g.replace(/\s+$/,"").replace(/[^\s]+$/,"").replace(/\s+$/,"");f.innerHTML=sc.encodeHTML(j)+'<span class="word-highlight">'+sc.encodeHTML(a)+"</span>"+sc.encodeHTML(g)}return k})},_reviewWord:function(){var a=this._wordKeys[0];this._currentWord=a;sc._("spell-current"+this._uniqueID).value=a;this._wordMatches=this._getTotalWordMatches();this._setSuggestionOptions();this._setContextBox()},_getTotalWordMatches:function(){var a=this,
b=this._currentWord.length,c=/^\w+$/,d=0;a._text.replace(RegExp(this._currentWord,"g"),function(e,f){c.test(a._text.substr(f+b,1))||d++;return e});return d},_begin:function(a){a.success&&!0===a.success&&(a.errors&&!0===a.errors?(sc._("spell-hidden"+this._uniqueID).value=this._text,this._canUndo=!1,this._wordObject=a.words,this._wordKeys=sc.objectKeys(this._wordObject),this._matchOffset=1,this._showReviewer(),this._reviewWord()):this._notifyMsg("noerrors"))},_handleXHR:function(a){a=sc.parseJSON(sc.trim(a.responseText));
!1!==a?this._begin(a):this._notifyMsg("servererror")},_sendData:function(){var a=this,b=sc.newXHR(),c=a._text,d;!1===b?a._notifyMsg("servererror"):2>c.length?a._notifyMsg("noerrors"):(d=a._settings.name+"=",d+=encodeURIComponent(c),d=d+"&"+sc.obj2string(a._settings.data),b.onreadystatechange=function(){4===b.readyState&&(200===b.status?a._handleXHR(b):a._notifyMsg("servererror"))},b.open("POST",a._serverURL,!0),b.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01"),b.setRequestHeader("X-Requested-With",
"XMLHttpRequest"),b.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),b.send(d))},_createHTML:function(){var a=document.createElement("div"),b=document.createElement("div"),c=document.createElement("div"),d=document.createElement("textarea");document.body.appendChild(a);a.className="spell-check-overlay";a.setAttribute("id","spell-overlay"+this._uniqueID);document.body.appendChild(b);b.className="spell-wrap";b.setAttribute("id","spell-modal"+this._uniqueID);b.innerHTML='<div class="spell-header">Spell Check</div><div id="spelling-inner'+
...
this._uniqueID+'"><option></option></select></div><div class="spell-changebtns"><button type="button" id="spell-change'+this._uniqueID+'">Change</button><button id="spell-change-all'+this._uniqueID+'">Change All</button><button type="button" id="spell-undo'+this._uniqueID+'">Undo</button></div><hr /><div class="close-box"><button type="button" id="spell-close'+this._uniqueID+'">Close</button></div></div>';document.body.appendChild(c);c.className="spell-msg";c.setAttribute("id","spell-msg"+this._uniqueID);
c.innerHTML='<div class="spell-header">Spell Check</div><div class="spell-msg-inner"><span id="spell-msg-text'+this._uniqueID+'"></span></div><div class="spell-msg-inner" id="spell-msg-close-box'+this._uniqueID+'"><button id="spell-msg-close'+this._uniqueID+'">OK</button></div>';document.body.appendChild(d);d.style.display="none";d.setAttribute("id","spell-hidden"+this._uniqueID)}};